...
data = """[1518-09-01 00:58] wakes up
...
# data = ['[1518-03-15 00:01] Guard #709 begins shift', '[1518-03-15 00:12] falls asleep', '[1518-03-15 00:53] wakes up',
#         '[1518-03-15 23:53] Guard #919 begins shift', '[1518-03-16 00:01] falls asleep', '[1518-03-16 00:09] wakes up',
#         '[1518-03-16 00:35] falls asleep', '[1518-03-16 00:43] wakes up', '[1518-03-18 00:01] Guard #999 begins shift']

current_guard_record = '.' * 60
current_guard_no = 0
...
for date in data:
    ...
    action = date[19:24]
    if action == 'Guard':
        # store previous guard record
        sleep_dict[idx] = [month, day, current_guard_no, current_guard_record]
        idx += 1
        current_guard_no = date[26:].split()[0]
        current_guard_record = '.' * 60
    elif action == 'falls':
        current_sleep_start = minute
    elif action == 'wakes':
        current_sleep_stop = minute
        # store this sleep for guard (there may be another)
        this_sleep_length = current_sleep_stop - current_sleep_start
        if current_guard_no in guard_sleeps.keys():
            guard_sleeps[current_guard_no] += this_sleep_length
        else:
            guard_sleeps[current_guard_no] = this_sleep_length

        this_sleep = ''
        for i in range(60):
            if i >= current_sleep_start and i < current_sleep_stop:
                this_sleep += '*'
            else:
                this_sleep += current_guard_record[i]
        current_guard_record = this_sleep

empty_minutes_sleep = [0 for _ in range(60)]

sleep_totals = {}

for k, v in sleep_dict.items():
    print(k, v)
    if v[2] in sleep_totals.keys():
        minutes_sleep = sleep_totals[v[2]]
        for i in range(60):
            if v[3][i] == '*':
                minutes_sleep[i] += 1
    else:
        minutes_sleep = empty_minutes_sleep.copy()
        for i in range(60):
            if v[3][i] == '*':
                minutes_sleep[i] += 1
        sleep_totals[v[2]] = minutes_sleep

highest_minute = 0
highest_minute_index = 0
laziest_guard = ''
for k, v in sleep_totals.items():
    for idx, sleeps in enumerate(v):
        if sleeps > highest_minute:
            highest_minute = sleeps
            highest_minute_index = idx
            laziest_guard = k

print(highest_minute_index, laziest_guard, highest_minute_index * int(laziest_guard))
# print(minutes_sleep)
#
# print(max(minutes_sleep), minutes_sleep.index(max(minutes_sleep)))
#
# guard_sleeps_sorted = sorted(guard_sleeps.items(), key=operator.itemgetter(1))
# print(guard_sleeps_sorted)
