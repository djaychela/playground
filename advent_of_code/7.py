...
data = data.split('\n')
action = ''
# init empty dict with all chars
pre_req = {char: '' for char in string.ascii_uppercase}
seen = set()
for d in data:
    pre = d[5]
    post = d[36]
...
for letter in seen:
    print(pre_req[letter])

while len(seen) >0:
    do_this_round = []
    for letter in seen:
        if len(pre_req[letter]) == 0:
            do_this_round.append(letter)
    this_one = sorted(do_this_round)[0]
    action += this_one

    # remove this_one from all keys
    for letter in seen:
        text = pre_req[letter]
        text = text.replace(this_one, '')
        pre_req[letter] = text


    seen.remove(this_one)

print(action)

# done when no other letters in pre_req
# when done, remove letter from others in pre-req
# when done, add a * to pre-req so it gets ignored
